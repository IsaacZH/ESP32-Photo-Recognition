...
def parse_response(response_text):    
    """
    Parse server response text and convert it to dictionary format.

    Parameters:
    response_text (str): The text of the server response.

    Returns:
    dict: A dictionary containing items and their counts.
    """
    item_count = {}
...
try:
    # Send POST request
    response = urequests.post(url, json=data, timeout=10)  # Set timeout to 10 seconds
